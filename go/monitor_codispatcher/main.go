...
const ConfDir = "/opt/codispatcher/etc"
...
func Query() {
	...
	for _, f := range confFiles {
		...
		tmp["{#CONF}"] = f
		tmp["{#DEBUGPORT}"] = strings.Split(conf.Web.DebugPort,":")[1]
		tmp["{#QUERYPORT}"] = strings.Split(conf.Web.Queryport,":")[1]
		result["data"][i] = tmp
		i++
	}
...
func Check(port int) {
	cmmand1 := fmt.Sprintf("netstat -anp|grep %d | grep codispatcher | awk '{print $4}'", port)
	...
	if err == nil {
		defer conn.Close()
		command2 := fmt.Sprintf("netstat -anp|grep %d | grep codispatcher | awk '{print $7}'", port)
